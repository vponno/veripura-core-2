...
export const generateComplianceChecklist = async (fileBase64: string, mimeType: string, fromCountry: string, toCountry: string): Promise<{ extractedData: ExtractedPOData, checklist: Omit<ChecklistItem, 'id' | 'status'>[] }> => {
 ...
 3.  **Generate Compliance Checklist:** Based on the extracted products, their identified HS codes, and the origin/destination countries, generate a detailed checklist of all required export and import documents. **IMPORTANT: Generate GENERIC document names WITHOUT country suffixes.** For example:
     - Use "Import Permit" NOT "Import Permit (Switzerland)"
     - Use "Customs Declaration" NOT "Customs Declaration (USA)"
     - Use "Health Certificate" NOT "Health Certificate for EU"
     Include general documents (invoices, packing lists, bills of lading) and specific ones based on HS codes or organic status. For each item, provide its name (generic, no country suffix), a brief description, the issuingAgency, and categorize it as 'Customs', 'Regulatory', 'Food Safety', 'Quality', 'Transport', 'Organic', or 'Other'.
