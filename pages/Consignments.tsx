...
const Consignments: React.FC = () => {
    ...
    const handleDelete = async (e: React.MouseEvent, id: string) => {
        ...
        setConfirmModal({
            ...
            message: 'Are you sure you want to archive this consignment? Data will be preserved for audits but hidden from this list.',
            variant: 'warning',
            onConfirm: async () => {
                try {
    ...
    const handleArchiveAll = async () => {
        ...
        setConfirmModal({
            isOpen: true,
            title: '⚠️ ARCHIVE ALL?',
            message: 'This will move ALL active consignments to the archive. This action cannot be easily undone via the UI.',
            variant: 'danger',
            onConfirm: async () => {
                try {
                    setConfirmModal(prev => ({ ...prev, isOpen: false }));
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            {loading ? (
                ...
                <div className="text-center py-12 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-dashed border-slate-300 dark:border-slate-700">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">No Active Consignments</h3>
                    <p className="text-slate-500 dark:text-slate-400 mb-4">You haven't registered any export consignments yet.</p>
                    <button
                        onClick={() => navigate('/register-consignment')}
                ...
                <div className="grid grid-cols-1 gap-4">
                    {consignments.map((item) => {
                        ...
                        return (
                            <div key={item.id} className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row gap-6 hover:shadow-md transition-shadow cursor-pointer group" onClick={() => navigate(`/register-consignment?id=${item.id}`)}>
                                <div className="flex-1">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className={`px-2 py-1 text-xs font-bold uppercase tracking-wide rounded ${item.status === 'Completed' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' :
                                                item.status === 'In Progress' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400'
                                                }`}>
                                                {item.status}
                                            </span>
                                    ...
                                    <div className="flex items-center gap-2 mb-4">
                                        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{item.exportFrom}</h3>
                                        <ArrowRight size={16} className="text-slate-400" />
                                        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{item.importTo}</h3>
                                    </div>
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    <button className="p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-full text-slate-400 transition-colors">
                                        <ArrowRight size={20} />
                                    </button>
                                    <button
                                        onClick={(e) => handleDelete(e, item.id!)}
                                        className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-200 rounded-full text-slate-300 dark:text-slate-600 transition-colors opacity-0 group-hover:opacity-100"
                                        title="Archive Consignment"
                                    >
                                        <Archive size={18} />
                                    </button>
